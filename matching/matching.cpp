...
cv::Mat DrawRegions(const cv::Mat &in_img,
  ...
  if (in_img.channels() == 1)
    cv::cvtColor(in_img,out_img,cv::COLOR_GRAY2RGB);
  else
  ...
  if (ReprojectToOneImage)
    {
      ...
      if (in_img1.channels() != 3)
        cv::cvtColor(in_img1,out_tmp1,cv::COLOR_GRAY2RGB);
      else
        out_tmp1=in_img1.clone();
      if (in_img2.channels() != 3)
        cv::cvtColor(in_img2,out_tmp2,cv::COLOR_GRAY2RGB);
      else
  ...
  else
    {
      ...
      //          cv::cvtColor(out_tmp1.clone(),out_tmp1,cv::COLOR_GRAY2RGB);
      ...
      //        cv::cvtColor(out_tmp2.clone(),out_tmp2,cv::COLOR_GRAY2RGB);
                         ...
                         const int r2,const cv::Scalar color1,const cv::Scalar color2)
{
  ...
  if (in_img.channels() !=3)
    {
      cv::cvtColor(in_img,out_img,cv::COLOR_GRAY2RGB);
      cv::cvtColor(in_img,tmpimage1,cv::COLOR_GRAY2RGB);
  ...
  if (conc_horiz)
    {
      ...
      if (tmpimage1.channels() !=3)
        {
          cv::cvtColor(tmpimage1,tmpimage1,cv::COLOR_GRAY2RGB);
        }
      std::vector<TentativeCorrespExt>::iterator ptrOut = matchings.TCList.begin();
