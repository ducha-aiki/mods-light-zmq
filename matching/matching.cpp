...
int LORANSACFiltering(TentativeCorrespListExt &in_corresp, TentativeCorrespListExt &ransac_corresp,double *H, const RANSACPars pars)
{
    ...
    const double SYM_CHECK_COEF =  3.0 * (pars.doSymmCheck > 0);
    if (pars.useF) {
        ...
        case SAMPSON_F:   {
            ...
            error_threshold = pars.err_threshold*pars.err_threshold;
            SymCheck_th = pars.err_threshold*pars.err_threshold * SYM_CHECK_COEF;
            break;
        }
        ...
        case SAMPSON:   {
            ...
            HDSidx1 = &HDsidx;
            error_threshold = pars.err_threshold*pars.err_threshold;
            SymCheck_th = pars.err_threshold * SYM_CHECK_COEF;
            break;
        }
        case SYMM_SQ_MAX:   {
            HDS1 = &HDsSymMaxSq;
        ...
        case SYMM_MAX:   {
            ...
            error_threshold = pars.err_threshold;
            SymCheck_th = 0;
            break;
        }
        case SYMM_SQ_SUM:   {
    ...
    int do_laf_check = pars.LAFCoef > 0;
    double *u2Ptr_p1 = new double[do_laf_check*tent_size*6], *u2_p1;
    u2_p1=u2Ptr_p1;
    double *u2Ptr_p2 = new double[do_laf_check*tent_size*6], *u2_p2;
    u2_p2=u2Ptr_p2;
    ...
    if (do_laf_check) {
        ...
        { //x1,y1,1
            ...
            *u2Ptr =  1.;
            ...
            //x1 + a12,y1 + a22,1
            *u2Ptr_p1 =  ptr1->first.reproj_kp.x + 3.0*ptr1->first.reproj_kp.s * ptr1->first.reproj_kp.a12;
            u2Ptr_p1++;
            *u2Ptr_p1 =  ptr1->first.reproj_kp.y + 3.0*ptr1->first.reproj_kp.s * ptr1->first.reproj_kp.a22;
            u2Ptr_p1++;
            *u2Ptr_p1 =  1.;
            u2Ptr_p1++;

            //x2 + a12,y2 + a22,1
            *u2Ptr_p1 = ptr1->second.reproj_kp.x + 3.0*ptr1->second.reproj_kp.s * ptr1->second.reproj_kp.a12;
            u2Ptr_p1++;
            *u2Ptr_p1 = ptr1->second.reproj_kp.y + 3.0*ptr1->second.reproj_kp.s * ptr1->second.reproj_kp.a22;
            u2Ptr_p1++;
            *u2Ptr_p1 =  1.;
            u2Ptr_p1++;
            ...
            //x1 + a11,y1 + a21,1
            *u2Ptr_p2 = ptr1->first.reproj_kp.x + 3.0*ptr1->first.reproj_kp.s * ptr1->first.reproj_kp.a11;
            u2Ptr_p2++;
            *u2Ptr_p2 = ptr1->first.reproj_kp.y + 3.0*ptr1->first.reproj_kp.s * ptr1->first.reproj_kp.a21;
            u2Ptr_p2++;
            *u2Ptr_p2 =  1.;
            u2Ptr_p2++;

            //x2 + a11,y2 + a21,1
            *u2Ptr_p2 = ptr1->second.reproj_kp.x + 3.0*ptr1->second.reproj_kp.s * ptr1->second.reproj_kp.a11;
            u2Ptr_p2++;
            *u2Ptr_p2 = ptr1->second.reproj_kp.y + 3.0*ptr1->second.reproj_kp.s * ptr1->second.reproj_kp.a21;
            u2Ptr_p2++;
            *u2Ptr_p2 =  1.;
            u2Ptr_p2++;
        }
    ...
    if (!(pars.useF)) //H
    {
        ...
        for (i=0; i<9; i++)
        {
            ransac_corresp.model[i]=HinvPtr[i];
            H[i] = HinvPtr[i];
        }
        ///
    ...
    else   //F
    {
        ...
        //        true_size = checked_corresp.TCList.size();
        for (i=0; i<9; i++)
            ransac_corresp.model[i]=Hloran[i];
    }
...
int HMatrixFiltering(TentativeCorrespListExt &in_corresp, TentativeCorrespListExt &true_corresp,double *H, const int isExtended, const RANSACPars pars)
{
    ...
    delete [] u2;
    delete [] inl2;
    true_corresp.model[0]=H[0];
    true_corresp.model[1]=H[3];
    true_corresp.model[2]=H[6];
    true_corresp.model[3]=H[1];
    true_corresp.model[4]=H[4];
    true_corresp.model[5]=H[7];
    true_corresp.model[6]=H[2];
    true_corresp.model[7]=H[5];
    true_corresp.model[8]=H[8];
    return true_size;
}
...
void DuplicateFiltering(TentativeCorrespListExt &in_corresp, const double r, const int mode)
{
    ...
    for (i=0; i<9; i++)
        unique_list.model[i] = in_corresp.model[i];
