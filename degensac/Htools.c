...
void HDsSymidx(const double *lin, const double * mu,
...
void HDsSymidxMax(const double *lin, const double * mu,
         const double *H, double *p, int len, int *idx, int siz) //Mishkin
{
   int i,mi;
   double Hinv[9],H1[9];
...
int all_HoriR_valid (double * us, int *idx)
{
   ...
   ori = (p[0]*c[0]+p[1]*c[1]+p[2]*c[2])*(q[0]*c[3]+q[1]*c[4]+q[2]*c[5]);
   if (ori*(p[0]*d[0]+p[1]*d[1]+p[2]*d[2])*(q[0]*d[3]+q[1]*d[4]+q[2]*d[5])<0)
      return 0;
                                                                                                        
   crossprod(p,c,d);
   crossprod(q,c+3,d+3);
                                                                                                              
   if (ori*(p[0]*a[0]+p[1]*a[1]+p[2]*a[2])*(q[0]*a[3]+q[1]*a[4]+q[2]*a[5])<0)
      return 0;
   if (ori*(p[0]*b[0]+p[1]*b[1]+p[2]*b[2])*(q[0]*b[3]+q[1]*b[4]+q[2]*b[5])<0)
      return 0;
   return 1;
}
