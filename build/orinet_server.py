...
def describe_patches(model, image, DO_CUDA = True, DESCR_OUT_DIM = 2, BATCH_SIZE = 512):
    h,w = image.shape
    n_patches = int(h/w)
    t = time.time()
...
parser = argparse.ArgumentParser(description='Local patch orientation server')
parser.add_argument('--no-cuda', action='store_true', default=False,
                    help='enables CUDA inference')
parser.add_argument('--gpu-id', default='0', type=str,
                    help='id(s) for CUDA_VISIBLE_DEVICES')
parser.add_argument('--port', default='5557', type=str,
                    help='id(s) for CUDA_VISIBLE_DEVICES')
