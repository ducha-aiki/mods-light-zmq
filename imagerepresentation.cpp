...
std::vector<std::vector<float> > DescribeWithTorchScript(torch::jit::script::Module& model,
                                                         torchscriptDescriptorParams par,
                                                         AffineRegionVector &kps,
                                                         SynthImage &temp_img1){

    int kp_size = kps.size() ;
    std::vector<std::vector<float> > out;
    int batch_size = par.batchSize;
    //  std::cout << kp_size << std::endl;
    if (kp_size <= batch_size){
        torch::Tensor patches;
        int odd_patch_size = par.patchSize ;
        if (kps.size() > 0){
            ExtractPatches2Tensor(kps,temp_img1, patches,
                                  par.mrSize,
                                  odd_patch_size,
                                  false,
                                  false,
            ...
            //patches.to(device);
            inputs.push_back(patches.to(device));
...
#pragma omp critical
            {
                ...
                int desc_size = output.size(1);
                for (int img_num=0; img_num<kps.size(); img_num++)
                {
                    std::vector<float> curr_desc(desc_size);
                    int offset = img_num*desc_size;
                    for (int i = 0; i < desc_size; ++i) {
                        const float v1 = out_ptr[i+offset];
                        curr_desc[i] = v1;
                    }
                    out.push_back(curr_desc);
                }
        ...
        for (int bi = 0; bi < num_batches; bi++) {
            ...
            AffineRegionVector current_kps(startIter, endIter);
            std::vector<std::vector<float> > current_out = DescribeWithTorchScript(model, par,
                                                                                   current_kps,
                                                                                   temp_img1);
            std::move(current_out.begin(), current_out.end(), std::back_inserter(out));
            std::advance(startIter, to_add);
...
void saveKP_KM_format(AffineKeypoint &ak, std::ostream &s) {
    ...
    cv::Mat A = (cv::Mat_<float>(2,2) << ak.a11, ak.a12, ak.a21, ak.a22);
...
#pragma omp parallel for schedule (dynamic,1)
        for (unsigned int synth=0; synth<n_synths; synth++)
        {
            ...
            {
                doExternalAffineAdaptation = (det_par.BaumbergParam.external_command.size() > 0) || (det_par.BaumbergParam.useTS);
            ...
            if (doExternalAffineAdaptation) {
                ...
                if (temp_kp1.size() > 0) {
                    ...
                    } else if (afShPar.useTS) {
                        //det_par.AffNetParam.mrSize = afShPar.mrSize;
                        std::vector<std::vector<float> > a11a21a22 = DescribeWithTorchScript(CNN_models["AffNet"],
                                det_par.AffNetParam,
                                temp_kp1,
                                temp_img1);
            ...
            if (curr_det.compare("ReadAffs") == 0){
                ...
                if (SIFT_like_desc) {
                    ...
                    } else if (dom_ori_par.useTS) {
                        std::vector<std::vector<float> > yx = DescribeWithTorchScript(CNN_models["OriNet"],
                                det_par.OriNetParam,
                                temp_kp1,
                                temp_img1);
            ...
            for (unsigned int i_desc=0; i_desc < synth_par[curr_det][synth].descriptors.size();i_desc++) {
                ...
                else if (curr_desc.compare("TorchScript")==0)
                {
                    if (temp_kp1_desc.size() > 0) {
                        //                        std::vector<std::vector<float> > descrs = DescribeWithZmq(desc_par.zmqDescParam,
                        //                                                                                  temp_kp1_desc,
                        //                                                                                  temp_img1);
                        std::vector<std::vector<float> > descrs = DescribeWithTorchScript(CNN_models["TorchScriptDescriptor"],
                                desc_par.torchDescParam,
                                temp_kp1_desc,
                                temp_img1);
                        ...
                        for (unsigned int kp_idx = 0; kp_idx < temp_kp1_desc.size(); kp_idx++) {
                            ...
                            temp_kp1_desc[kp_idx].desc.type=DESC_TORCHSCRIPT;
                        }
                    }
                }
...
AffineRegionVector ImageRepresentation::PreLoadRegionsNPZ(std::string fname) {
    ...
    std::string desc_name = "TorchScript";
...
void ImageRepresentation::LoadRegionsNPZ(std::string fname) {
    ...
    AddRegions(avr,"ReadAffs", "TorchScript");
