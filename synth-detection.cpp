...
int DetectAffineShapeExt(AffineRegionList &in_kp_list,
  ...
  ExtractPatchesColumn(in_kp_list,img, all_patches,
                     par.mrSize,
                     patchSize ,
                     false,
                     false,
  ...
  if (focikp.is_open()) {
      ...
      for (int i = 0; i < in_kp_list.size(); i++) {
          ...
          double s1 = sqrt(fabs(a11*a22 - a11*a21));
          ...
          if (interpolateCheckBorders(img.pixels.cols,img.pixels.rows,
                                      ...
                                      (float) a21,
                                      (float) a22,
                                      s1*par.mrSize * in_kp_list[i].det_kp.s,
                                      s1*par.mrSize * in_kp_list[i].det_kp.s) ) {
              continue;
            }
          ...
          temp_region.det_kp.s = temp_region.det_kp.s * s1;
          temp_kp_list.push_back(temp_region);
